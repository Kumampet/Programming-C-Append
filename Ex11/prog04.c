...
int main(){
  ...
  while(1){
    ...
    //----Record型構造体の中身の入力----
    i=scanf("%d%s%d%d%d", &record.id, &record.name,
	    &record.scorep[0], &record.scorep[1], &record.scorep[2]);
    if(i==5){
      if (insert(record) == NULL){//----データ挿入関数へジャンプ----
	printf("\nData %s(%d) is already on the list\n", record.name, record.id);
...
NodePointer delete(int id){
  ...
  if((n = finditem(id)) != NULL){//該当のIDのポインタを引く
    //n = n - 1;//該当ポインタの一つ前のアドレスを参照値とする
    delnode = n->next;//戻したアドレスと基準とし、次のノード(つまり削除するノード)をdelnodeとして代入
    n->next = delnode->next;
    free(delnode);
    return n;
  }else return NULL;
